...
async function runOrchestration(taskName, taskDescription) {
  ...
  try {
    ...
    const rawErrorMessage = String(error && error.message ? error.message : 'Unknown error');
    const sanitizedErrorMessage = rawErrorMessage.replace(/[<>:"/\\|?*\x00-\x1F]/g, '_');
    const sanitizedErrorStack = typeof error?.stack === 'string'
      ? error.stack.replace(/[<>:"/\\|?*\x00-\x1F]/g, '_')
      : undefined;

    run.summary = {
      success: false,
      outcome: 'Orchestration failed',
      errors: [sanitizedErrorMessage, sanitizedErrorStack].filter(Boolean)
    };
